...
(define-primitive (fx* tail? si env arg1 arg2)
  ...
  (define (out1 expr const)
	(emit-expr #f si env expr)
	(emit "    imull $~a, %eax" const))  ; シフト必要なし
  (cond ((fixnum? arg2) (out1 arg1 arg2))
		((fixnum? arg1) (out1 arg2 arg1))
		(else (out2))))
...
(define (emit-heap-allocation expr)
  ...
  (cond ((fixnum? expr)
		...
		((eq? expr 'eax)  ; eax に入っているのは内部表現の数値じゃなく実際の数値であること
		 (emit "    addl $~a, %eax" (- HEAP-ALIGN 1))
		 (emit "    andl $~a, %eax" (- HEAP-ALIGN))
		 (emit "    addl %ebp, %eax")
		 (emit "    xchgl %eax, %ebp")  ; ebp: ヒープの次の空き領域
		 (emit "    movl %eax, %edx"))  ; edx: 確保したヒープ領域
		(else
...
(define-primitive (make-vector tail? si env n . v)
  (let ((v (cond ((null? v) #f)
	...
	(emit "    movl ~a(%esp), %ebx" (- si wordsize))  ; ebx = 要素数 (<< fxshift)
	(emit "    movl %ebx, ~a(%edx)" vector_num)

	(emit "    movl ~a(%esp), %eax" si)  ; 初期要素
	(emit "    lea ~a(%edx), %edi" vector_buf)  ; 配列要素
	(emit "    testl %ebx, %ebx")
	(emit-jmp l-test)
	(emit-label l-loop)
	(emit "    movl %eax, (%edi)")
...
(define-primitive (make-string tail? si env n . v)
  (let ((v (cond ((null? v) #\space)
	...
	(emit-heap-allocation 'eax)
	(emit "    movl ~a(%esp), %ebx" (- si wordsize))  ; ebx = 要素数
	(emit "    movl %ebx, ~a(%edx)" string_size)

	(emit "    movl ~a(%esp), %eax" si)  ; 初期要素
	(emit "    shrl $~a, %eax" charshift)
	(emit "    lea ~a(%edx), %edi" string_buf)  ; 配列要素
	(emit "    testl %ebx, %ebx")
	(emit-jmp l-test)
	(emit-label l-loop)
	(emit "    movb %al, (%edi)")
...
(define-primitive (string-set! tail? si env str idx val)
  ...
  (emit "    movl %eax, %edx")
  (emit "    movl ~a(%esp), %eax" si)
  (emit "    shrl $~a, %eax" charshift)  ; 文字の内部表現からアスキーコードへ
